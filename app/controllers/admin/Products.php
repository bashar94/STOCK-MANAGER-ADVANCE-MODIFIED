...
class Products extends MY_Controller
{
    ...
    function add($id = NULL)
    {
        ...
        $this->form_validation->set_rules('damage_unit', 'Damage Unit', 'numeric');
        $this->form_validation->set_rules('opening_stock', 'Opening Stock', 'numeric');
        $this->form_validation->set_rules('closing_stock', 'Closing Stock', 'numeric');
        $this->form_validation->set_rules('receiving_quantity', 'Receiving Quantity', 'numeric');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'manufacturing' => $this->input->post('manufacturing') ? $this->sma->fsd($this->input->post('manufacturing')) : NULL,
                'expire' => $this->input->post('expire') ? $this->sma->fsd($this->input->post('expire')) : NULL,
                'receive_date' => $this->input->post('receive_date') ? $this->sma->fsd($this->input->post('receive_date')) : NULL,
    ...
    function delete($id = NULL)
    {
        ...
        $product = $this->products_model->getProductByID($id)->code;
        $this->products_model->deleteExistingProductByCode($product);
    ...
    function finalize_count($id)
    {
     ...
     function modal_view_stock($id = NULL)
    {
        $this->sma->checkPermissions('index', TRUE);
    ...
    function add_existing($id = NULL)
    {
        $this->sma->checkPermissions();
        $this->load->helper('security');
       
        
        $this->form_validation->set_rules('code', lang("product_code"), 'required|alpha_dash');
        $this->form_validation->set_rules('receiving_quantity', 'Receiving Quantity', 'numeric');
        ...
        if ($this->form_validation->run() == true) {
            $data = array(
                ...
                'receive_date' => $this->input->post('receive_date') ? $this->sma->fsd($this->input->post('receive_date')) : NULL,
                'receiving_quantity' => $this->input->post('receiving_quantity'),
                'opening_stock' => $this->products_model->getOpeningStock($this->input->post('code')),

            );   
        }

        if ($this->form_validation->run() == true && $this->products_model->addExistingProduct($data)) {
            $this->session->set_flashdata('message', lang("product_added"));
            admin_redirect('products');
        } else {
            
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            // $this->data['test'] = $this->products_model->getOpeningStock(1);
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => 'Add Existing Product'));
            $meta = array('page_title' => lang('add_product'), 'bc' => $bc);
            $this->page_construct('products/add_existing', $meta, $this->data);
        }
    }

    function deleteExisting($id = NULL)
    {
        $this->sma->checkPermissions(NULL, TRUE);
        ...
        if ($this->products_model->deleteExistingProduct($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("product_deleted")));
        }
        $this->sma->send_json(array('error' => 1, 'msg' => lang("ajax_error")));
